...
EasyAccuracy: 34.97%Submissions: 67K+Points: 2
...
class Solution{
    Node* func(Node* root, Node* x, vector<Node*> &vec){
        
        if(root==NULL) return root;
        
        if(root->left) func(root->left, x, vec);
        vec.push_back(root);
        if(root->right) func(root->right, x, vec);
    }
  public:
    ...
    Node * inOrderSuccessor(Node *root, Node *x)
    {
        ...
        for(int i=0; i<v.size(); i++){
            if(v[i]==x && i+1<v.size()) return v[i+1];
        }
        return NULL;
    }
};
