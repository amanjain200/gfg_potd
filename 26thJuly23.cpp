...
MediumAccuracy: 35.06%Submissions: 68K+Points: 4
...
bool func(Node* root, stack<int> &q, int node){
    if(root==NULL) return true;
    
    if(root->data==node) return false;
    
    if(root->left){
        q.push(root->left->data);
        if(func(root->left, q, node)==false) return false;
        q.pop();
    }
    
    if(root->right){
        q.push(root->right->data);
        if(func(root->right, q, node)==false) return false;
        q.pop();
    }
